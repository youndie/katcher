...
class SymbolicationService(
    ...
    suspend fun processCrash(
        ...
        println(
            "SymbolicationService.processCrash - Found symbol map: type=${mapMetadata.type}, filePath=${mapMetadata.filePath}",
        )
        val strategy =
            strategies[mapMetadata.type]
                ?: run {
                    println(
                        "SymbolicationService.processCrash - No strategy found for type=${mapMetadata.type}, returning raw stacktrace",
                    )
                    return rawStacktrace
                }

        println(
            "SymbolicationService.processCrash - Starting symbolication with strategy=${strategy::class.simpleName}",
        )
